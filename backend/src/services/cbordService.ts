...
import { DayMenu, DiningHall, MealPeriod, MenuItem } from "../types/dining";
...
class CbordService {
   ...
   */
  private async getUnitStatusFromHours(cbordUnitId: number): Promise<boolean | null> {
    try {
   ...
   * @returns true if open, false if closed, null if unable to determine
   */
  private calculateStatusFromHours(html: string, unitId?: number): boolean | null {
    try {
      ...
      const todayRows: Array<{ openTime: number | null; closeTime: number | null; isDanger: boolean }> = [];
   ...
   */
  private async getUnitStatusFromAPI(): Promise<Map<number, boolean>> {
    const unitStatusMap: Map<number, boolean> = new Map();
    
    try {
      console.log(`[Status] Fetching hours for ${DINING_HALLS.length} units...`);
      
      // Fetch hours for all units in parallel
      const statusPromises = DINING_HALLS.map(async (hall) => {
        try {
  ...
  async getDiningHalls(): Promise<DiningHall[]> {
    const hallsWithStatus: DiningHall[] = [];
  ...
  private parseMenuItems(html: string): MenuItem[] {
    ...
    itemRows.forEach((row) => {
      ...
      let detailOid: number | undefined;
   ...
   */
  async getItemNutrition(detailOid: number, cbordUnitId?: number, menuId?: number): Promise<MenuItem["nutrition"] | null> {
    try {
      ...
      if (typeof response.data === 'object' && response.data.panels) {
        ...
        const nutritionPanel = response.data.panels.find((p: any) => 
          p.id === 'nutritionPanel' || p.id === 'itemPanel' || p.html
        );
   ...
   */
  private parseNutritionLabel(html: string): MenuItem["nutrition"] | null {
    try {
      const root = parse(html);
      const nutrition: NonNullable<MenuItem["nutrition"]> = {};
